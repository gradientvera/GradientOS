...
rec {
  ...
  tasks = {
    ...
    nixMakeSymlinkToFile =
      ...
      tasks.nixMakeSymlinkCustom {
        inherit
          ...
          moduleArgs
          ;
        destPath = "${destPath}/${baseNameOf srcPath}";
      };

    nixMakeSymlinkCustom =
      {
        pkg,
        srcPath,
        destPath,
    ...
    nixMakeSymlinkToMainExe =
      ...
      in
      tasks.nixMakeSymlinkCustom {
        inherit pkg taskArgs moduleArgs;
        srcPath = builtins.replaceStrings [ (toString pkg) ] [ "" ] pkgExe;
        destPath = "${destPath}/${baseNameOf pkgExeName}";
      };

    nixMakeSymlinkToMainExeGlibc =
      {
        pkg,
        glibc,
        destPath,
      ...
      let
        ...
        pkgExeName = baseNameOf pkgExe;
      in
      tasks.nixMakeSymlinkCustomGlibc {
        inherit
          pkg
          glibc
          taskArgs
        ...
        task = {
          name =
            if builtins.isList name then
              "Installing opkg packages ${package}"
            else
              "Installing opkg package ${package}";
        }
        // taskArgs;
    ...
    pip =
      ...
      mkAnsibleTask {
        task = {
          name =
            if builtins.isList name then
              "Installing Python pip packages ${package}"
            else
              "Installing Python pip package ${package}";
        }
        // taskArgs;
