...
GetVars
mosquitto_pub --quiet -h "$MQTT_HOST" -p "$MQTT_PORT" -t "homeassistant/camera/${VALETUDO_ID}/${VALETUDO_ID}_gradient_camera/config" -r -m "${AUTODISCOVER}"
...
while true; do
    ...
    magick /tmp/bmp_YUV420_1.bmp /tmp/bmp_YUV420_1.jpg >> /dev/null || continue

    mosquitto_pub --quiet -h "$MQTT_HOST" -p "$MQTT_PORT" -t $TOPIC -r -f /tmp/bmp_YUV420_1.jpg || continue

    rm /tmp/bmp_YUV420_1.bmp /tmp/bmp_YUV420_1.jpg >> /dev/null || continue
