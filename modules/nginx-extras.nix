...
in
{
  ...
  options = {
    gradient.nginx.enableQuic = lib.mkEnableOption "Whether to enable QUIC support in all vhosts.";
    gradient.nginx.enablekTLS = lib.mkEnableOption "Whether to enable kTLS support in all vhosts.";
    gradient.nginx.enableBlockAIBots = lib.mkEnableOption "Whether to block AI scraper blocks.";
      ...
      type = lib.types.attrsOf (
        lib.types.submodule {
          config = {
            ...
            locations."= /robots.txt" = lib.mkIf cfg.enableBlockAIBots (
              lib.mkDefault {
                alias = robotsTxt;
              }
            );
