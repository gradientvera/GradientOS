...
let
  ...
  filterFileAttrs = (files: attrsets.filterAttrs (_: v: v ? C && v.C.doCheck) files);
  settingsToListOfStrings = (
    mkScript: settings:
    (attrsets.mapAttrsToList (
      _: files:
      (builtins.concatStringsSep "\n" (attrsets.mapAttrsToList mkScript (filterFileAttrs files)))
    ) settings)
  );
  makeScriptFromSettings = (
    mkScript: builtins.concatStringsSep "\n" (settingsToListOfStrings mkScript tmpfiles-settings)
  );
  checkScriptFromFileSetting = (
    target: args:
    let
      source = args.C.argument;
    in
    ''
      RED="\033[0;31m"
      PURPLE="\033[1;35m"
