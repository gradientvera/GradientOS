    ...
    type = lib.types.attrsOf (
      lib.types.submodule (
        ...
        let
          ...
          childrenContainers = (
            lib.filterAttrs (
              n: v: n != name && (builtins.any (x: x == "container:${name}") (v.networks or [ ]))
            ) ociCfg
          );
          # Which containers we're attached to --network=container:<name>
          parentContainers = (
            builtins.map (n: lib.strings.removePrefix "container:" n) (
              builtins.filter (n: lib.strings.hasPrefix "container:" n) config.networks
            )
          );
        ...
        in
        {
          ...
          config = {
            ...
            labels = lib.mkIf cfg.autoUpdate (
              lib.mkDefault {
