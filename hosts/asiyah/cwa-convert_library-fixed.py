# By crocodilestick, taken from https://github.com/crocodilestick/Calibre-Web-Automated/blob/19960a926eaef4461d45a4634cd90ca6ba8f0ded/scripts/convert_library.py
# Licensed under the GNU General Public License Version 3, full text at https://github.com/crocodilestick/Calibre-Web-Automated/blob/19960a926eaef4461d45a4634cd90ca6ba8f0ded/LICENSE
...
# 5 minute timeout to process a book
timeout = 5 * 60
...
class LibraryConverter:
    ...
    def get_books_to_convert(self):
        ...
        for file in files_already_in_target_format:
            # Exclude by book directory, as they may have different filenames...
            directory = os.path.dirname(file)
            exclusion_list.append(directory) # Adding books with a file already in the target format to the exclusion list
        ...
        for format in self.hierarchy_of_success:
            ...
            if len(files_in_format) > 0:
                for file in files_in_format:
                    directory = os.path.dirname(file)
                    if directory not in exclusion_list:
                        to_convert.append(file)
                        exclusion_list.append(directory)

        # Shuffle so that it doesn't always convert in the same order.
        random.shuffle(to_convert)
        return to_convert
    ...
    def convert_library(self):
        for file in self.to_convert:
            ...
            try: # Get Calibre Library Book ID by directory, book files may not contain it
                book_id = (re.search(r'\(\d*\)$', directory).group(0))[1:-1] # type: ignore
                print_and_log(f"[convert-library]: ({self.current_book}/{len(self.to_convert)}) Converting {filename} (ID: {book_id}) from {file_extension} format to {self.target_format} format...")
            except Exception as e:
                print_and_log(f"[convert-library]: ({self.current_book}/{len(self.to_convert)}) A Calibre Library Book ID could not be determined for {directory}. Make sure the structure of your calibre library matches the following example:\n")
                ...
                print_and_log(f"[convert-library]: ({self.current_book}/{len(self.to_convert)}) Error:\n{e}")
