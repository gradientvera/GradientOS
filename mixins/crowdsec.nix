...
let
  ...
  cfg = config.gradient.crowdsec;
in
{
  # Generic hook: hosts can add their own notification names (defined
  # elsewhere via environment.etc) to the ban-notification profiles.
  options.gradient.crowdsec = {
    extraNotifications = lib.mkOption {
      type = lib.types.listOf lib.types.str;
      default = [ ];
      description = "Extra crowdsec notification names added to ban-notification profiles.";
    };
  };
