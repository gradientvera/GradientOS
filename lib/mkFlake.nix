  ...
  packages =
    let
      configurations = builtins.filter (
        x: (builtins.length x.generators or [ ]) != 0
      ) gradientosConfigurations;
      x86_64-linux-configurations = builtins.filter (
        x: x.system or "x86_64-linux" == "x86_64-linux"
      ) configurations;
      aarch64-linux-configurations = builtins.filter (
        x: x.system or "x86_64-linux" == "aarch64-linux"
      ) configurations;
      generate =
        config:
        (map (x: {
          name = config.name + "-" + x;
          value = lib.gradientosSystemGenerator (config // { format = x; });
        }) (nixpkgsLib.lists.unique config.generators or [ ]));
      generateMany =
        configs: builtins.listToAttrs (nixpkgsLib.lists.flatten (map (x: generate x) configs));
    in
    (nixpkgsLib.attrsets.recursiveUpdate ({
